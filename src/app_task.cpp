...
// gpio info from overlay nrf7002dk_nrf5340_cpuapp.overlay
static const struct gpio_dt_spec upbuttonDevice = GPIO_DT_SPEC_GET(DT_ALIAS(upbutton), gpios); //need to replace LED0_NODE
static const struct gpio_dt_spec downbuttonDevice = GPIO_DT_SPEC_GET(DT_ALIAS(downbutton), gpios);

//Configure the buttonDevices to set the GPIO pins to ACTIVE
//Once GPIO pins are set to HIGH, the logical value will be 1
int ret0 = gpio_pin_configure_dt(&upbuttonDevice, GPIO_OUTPUT_ACTIVE);
int ret1 = gpio_pin_configure_dt(&downbuttonDevice, GPIO_OUTPUT_ACTIVE);
...
CHIP_ERROR AppTask::Init()
{
	...
	// k_timer_init(&button_timer, &AppTask::, NULL);
...
// void AppTask::SetGPIOForControlButtons(gpio_dt_spec *buttonDevice){
	
// 	gpio_pin_set_dt(buttonDevice, GPIO_OUTPUT_HIGH);

// 	//wait 3 seconds
// 	/* start one shot timer that expires after 3 sec */
// 	// k_timer_start(&button_timer, K_SECONDS(3), K_NO_WAIT);

// 	k_msleep(3000);
	
// 	gpio_pin_set_dt(buttonDevice, GPIO_OUTPUT_LOW);
// }

// void AppTask::SetGPIOForControlButtonsLow(k_timer *timer){
// 	gpio_pin_set_dt(buttonDevice, GPIO_OUTPUT_LOW);
// }
...
{
	//The following is going to simulate pressing the up/down buttons
	//as if the user were pressing the buttons on the control panel
	if (action == PWMDevice::ON_ACTION) {
		LOG_INF("Turn On Action has been initiated");
		// Setting the upbuttonDevice to HIGH to move the desk up
		gpio_pin_set_dt(&upbuttonDevice, GPIO_OUTPUT_HIGH);
		LOG_INF("SETTING THE UP BUTTON GPIO TO HIGH, WAITING 3 SECONDS");
		// the standing desk will move fully up once button is pressed for 3 seconds
		k_msleep(3000);
		LOG_INF("AFTER 3 SECONDS, SETTING UP BUTTON GPIO TO LOW");
		// setting the gpio pin to LOW to turn off button press
		gpio_pin_set_dt(&upbuttonDevice, GPIO_OUTPUT_LOW);
	} else if (action == PWMDevice::OFF_ACTION) {
		LOG_INF("Turn Off Action has been initiated");
		// Setting the downbuttonDevice to HIGH to move the desk down
		gpio_pin_set_dt(&downbuttonDevice, GPIO_OUTPUT_HIGH);
		LOG_INF("SETTING THE DOWN BUTTON GPIO TO HIGH, WAITING 3 SECONDS");
		// the standing desk will move fully down once button is pressed for 3 seconds
		k_msleep(3000);
		LOG_INF("AFTER 3 SECONDS, SETTING DOWN BUTTON GPIO TO LOW");
		gpio_pin_set_dt(&downbuttonDevice, GPIO_OUTPUT_LOW);
